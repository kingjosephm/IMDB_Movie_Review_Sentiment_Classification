...
class clean_text:
    ...
    def correct_spelling(self, sentence):
        '''
        Corrects word misspellings in string of text
        :param sentence: string, text
        :return: str
        '''
        assert isinstance(sentence, str)
    ...
    def run(self, text, no_stop_words=True, correct_spelling=False, lemmatize=True, min_char_word=None, min_word_freq=None):
        '''
        ...
        if correct_spelling:
            text = text.apply(lambda x: self.correct_spelling(x))
